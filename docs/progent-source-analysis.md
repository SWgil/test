...
두 번째가 진짜 방어인데 off다 (아래 5.3 참고).

주의할 점 하나: 갱신은 **누적이 아니라 통째 재생성**이다. 새 정책 적용 전에 `delete_generated_policy()`가
priority 100 규칙을 전부 지우고 LLM의 새 출력으로 교체한다(`tool.py:484-490`). 권한이 한 방향으로 쌓이는
래칫 구조가 아니며, 갱신마다 좁아질 수도 넓어질 수도 있다.

### 5.3 "기본 설정"의 두 가지 의미

| | 갱신 `SECAGENT_UPDATE` | 축소증명 `SECAGENT_ONLY_ALLOW_NARROW` |
|---|---|---|
| 라이브러리/AgentDojo 코드 기본값 | `tool_execution.py:121` → **`"False"`** (꺼짐) | `False` |
| 논문 실험 스크립트 `run.sh` | `export SECAGENT_UPDATE="True"` | 주석 처리 (꺼짐) |

코드 기본값만 보면 갱신은 아예 안 돌고, 그 상태는 사용자 질의 하나로 만든 **정적 정책**이라 생성 이후
완전히 결정론적이다. 문제는 **논문이 헤드라인으로 돌리는 조합이 "갱신 켜고 + 축소증명 끈" 설정**이라는 점이다.
(`real-world-agents`/MCP 프록시 경로는 `SECAGENT_UPDATE` 기본값이 `"True"`이지만 전체가 `ENABLE_SECAGENT`로
게이팅되며, 이 역시 기본 `False`다.)

갱신을 켰을 때 공격자가 넘어야 할 관문은 넷이다.
1. 인젝션을 툴 결과에 삽입 (위협모델상 주어짐)
2. `decide_whether_to_update` 통과 — 그런데 이 게이트는 "정보 수집형 호출이면 갱신하라"고 프롬프트되어 있어
   **인젝션이 유입되는 바로 그 호출(메일/채널 읽기)에서 열린다.** 방어라기보다 통로에 가깝다.
3. 정책 재생성 LLM 설득 — 유일한 실질 방어이며 그 실체는 프롬프트 문장 하나다
4. 넓어진 정책이 공격 툴+인자를 실제로 커버

3번이 soft하다는 것이 핵심이다. 이 경로에서 보안은 "두 번째 LLM의 판단"으로 환원되고,
`ONLY_ALLOW_NARROW`만이 그것을 형식 증명으로 대체한다. 다만 이 두 번째 LLM은 에이전트 본체보다
공격 표면이 좁다 — 툴 접근 없음, 단일 턴, 출력이 JSON Schema로 제약됨.
...
- **정책 갱신이 신뢰 불가 데이터를 먹는다.** 갱신을 켜고 `ONLY_ALLOW_NARROW`를 끈 논문 실험 설정에서는
  인젝션된 툴 결과가 정책을 넓히도록 유도할 여지가 남는다 (5.3). 집행 계층 자체는 결정론적이지만,
  이 경로에서만은 "모델을 신뢰하지 않는다"는 성격이 성립하지 않는다.
- **Z3 분석 코드 대부분이 런타임에 연결되어 있지 않다.** `security_policy_analysis()`(규칙 충돌 검사)와
  `policy_type_check.security_policy_type_check()`는 각 파일의 `__main__` 데모에서만 호출된다.
  `tool.py:121`의 동명 함수는 본문이 `# check()`로 주석 처리된 빈 스텁이고 호출부도 없다.
  실제로 배선된 것은 `security_policy_subset_check()` 하나뿐이며 그마저 `ONLY_ALLOW_NARROW` 분기 안에 있다.
  즉 `role_analyzer.py`(900 LOC) + `policy_analysis.py`(441 LOC)는 기본 실행 경로에서 사실상 죽은 코드다.
